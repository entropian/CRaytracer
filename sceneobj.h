        ...
        while(ix != ix_stop && iy != iy_stop && iz != iz_stop)
        {
            ...
            if(rg->cells[cell_index].size > 0)
            {
                ...
                for(int i = 0; i < rg->cells[cell_index].size; i++)
                {
                    ...
                    tmp_t = rayIntersectObject(&tmp_sr, so->obj_ptrs[index], so->obj_types[index], ray);
                    if(tmp_t < obj_t)
                    {
                        obj_t = tmp_t;
                        obj_sr = tmp_sr;
    ...
    if(hits_grid)
    {
        ...
        while(ix != ix_stop && iy != iy_stop && iz != iz_stop)                
        {
            ...
            if(rg->cells[cell_index].size > 0)
            {
                ...
                {
                    int index = IntVector_get(&(rg->cells[cell_index]), i);
                    t = shadowRayIntersectObject(so->obj_ptrs[index], so->obj_types[index], shadow_ray);
                    if(t < TMAX)
                    {
                        return t;
                    }
...
float BVHIntersectTest(ShadeRec* sr, const SceneObjects* so, const BVHNode* tree, const Ray ray)
{
    if(tree->type == LEAF)
    {
        return rayIntersectObject(sr, tree->obj_ptr, tree->obj_type, ray);
    }else
    {
...
float BVHShadowIntersectTest(const BVHNode* tree, const Ray ray)
{
    if(tree->type == LEAF)
    {
        return shadowRayIntersectObject(tree->obj_ptr, tree->obj_type, ray);
    }else
    {
        float t1 = TMAX, t2 = TMAX;
        if(rayIntersectAABB(&(tree->left->aabb), ray) < TMAX)
...
void mvNonGridObjToStart(SceneObjects *so)
{
    ...
    for(int i = 0; i < so->num_obj; ++i)
    {
        if(!isGridObjType(so->obj_types[i]))
        {
            printf("here\n");
            ObjectType tmp_type;
            void* tmp_ptr;    
            tmp_ptr = so->obj_ptrs[i];
            tmp_type = so->obj_types[i];
            so->obj_ptrs[i] = so->obj_ptrs[j];
            so->obj_types[i] = so->obj_types[j];
            so->obj_ptrs[j] = tmp_ptr;
            so->obj_types[j] = tmp_type;
            ++j;
        }
    }
